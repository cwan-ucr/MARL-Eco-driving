...
parser.add_argument('--seed', type=int, default=2524, help='random seed')
...
parser.add_argument('--Max_episode', type=int, default=50, help='Max training episode')
...
parser.add_argument('--buffer_size', type=int, default=500000, help='Capacity of replay buffer')
...
def run_simulations(env, agent, total_steps, ep_i, opt):
    ...
    ep_CAVPR = env.CAV_number / env.discharge_number if env.discharge_number > 0 else 0
    ...
    ep_matrix = np.array([ep_fuel, ep_comfort, ep_stop, ep_tet, ep_tit, ep_dt, ep_tt, ep_q,
                          ...
                          ep_reward, actor_loss, critic_loss, alpha_loss, ep_CAVPR])
...
def train(opt):
    ...
    sumo_cmd = set_sumo(gui=True, sumocfg_file_name=cfg_file, max_steps=600)
